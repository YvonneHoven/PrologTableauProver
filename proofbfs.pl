...
prove(A, '|',L, C):- C\=[_], C\=[not,_], asst(A), wrt(C, '-'), nl, findall([Z], assprove(Z), AS), check(AS), writeln("conclusion solving:"), prove(C, '-'), findall([Y], toprove(Y, '+'), TP), findall([X], toprove(X, '-'), FP), check(TP, FP), nl, prepareAnswer(L).
